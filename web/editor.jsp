...
<html>
<head>
<title>Editing <%=user%> &#64; ngumbi.com</title>
<%
...
body {
    ...
    color: <%=textColor%>;
    font-family:<%=fontFamily%>;
    height: 95%; /* this is the key for making something within the body like the edit bar on right be 100% height */
}
-->
</style>
...
String userQuery = "SELECT user_id, username FROM users WHERE username = '"+user+"'";
...
if (!rsuser.next()){ //username doesn't exist
    ...
    */
	...
	//For every state other than state1, check login info so it's always verified before any SQL is executed
            ...
            if (logon.getUserID().equals(user) && logon.getSecure()){
                    ...
                    if (state.equals("2")){
                        ...
                        if (fromstate.equals("3a") || fromstate.equals("3a_1") || fromstate.equals("3a_2")){
                            ...
                            if (inputCheckOk) {
                                ...
                                //		**ADMIN UPDATE**
                                helperMethods.adminUpdate(user, "edit Add");
                        ...
                        if (fromstate.equals("3e_1") || fromstate.equals("3e_2")){
                            ...
                            //		**ADMIN UPDATE**
                            helperMethods.adminUpdate(user, "edit Edit");
                        ...
                        //From State 3d, Delete a Link
                            ...
                            //		**ADMIN UPDATE**
                            helperMethods.adminUpdate(user, "edit Delete");
                        ...
                        //From State 3r_1, 3r_2, Rename a Category
                            ...
                            if (!catnew.equals(cat)){
                                ...
                                if (inputCheckOk){ //inputs are ok, not bad characters or dups and there was a change
                                    ...
                                    //		**ADMIN UPDATE**
                                    helperMethods.adminUpdate(user, "edit Rename");
                        ...
                        //From State 3g, Change Search option
                            ...
                            if (fromstate.equals("3g")	&& tempSearchInt > 0 && tempSearchInt < 10 ){
                                ...
                                //		**ADMIN UPDATE**
                                helperMethods.adminUpdate(user, "edit Search");
                            }
                        }
                        ...
                        //From State 3p or 3p_1, Change password option
                            ...
                            if (goodPass){
                                ...
                                //		**ADMIN UPDATE**
                                helperMethods.adminUpdate(user, "edit Password");
                            }
                            else {
	...
	if (fromstate.equals("3dj")){ //was deleted, so don't attempt jumpstation code
            ...
            helperMethods.adminUpdate(user, "edit DeleteAcct");
            %>
            <div class="main">
                <jsp:include page="inc_ngumbi_title_unlinked.jsp" />
		...
		*/
		...
		if(rs.next()){
			...
			if( currCat.equals("")){//check if first link is a category-less one
                            ...
                            if (state.equals("3e") || state.equals("3d") || state.equals("3m")){
                                ...
                                %><a href="editor.jsp?user=<%=user%>&state=<%=state2%>&fromstate=<%=fromstate%>&user_link_id=<%=currLinkId%>&cat=<%=URLEncoder.encode(currCat,"UTF-8")%>&sub_cat_rank=<%=currSubCatRank%>"><%=rs.getString("link_name")%></a>&nbsp;&nbsp;<%
                            }
				...
				if (state.equals("3r")){ //print category name as a link for user to pick when modifying category
					...
					String state3 = "";
					state3 = state+"_1";
					%><a href="editor.jsp?user=<%=user%>&state=<%=state3%>&fromstate=<%=fromstate%>&cat=<%=URLEncoder.encode(currCat,"UTF-8")%>"><%=currCat%></a><%
				}
		...
		%>
		<td style="width: 200px; background-color: #ffc; padding: 20px 20px; vertical-align: middle;">
                    <%
		...
		//state 2, display main edit options
                    ...
                    <div style="text-align: center;">
                        <a href="editor.jsp?user=<%=user%>&state=3a&fromstate=<%=fromstate%>" >Add a link</a><br>
                        <a href="editor.jsp?user=<%=user%>&state=3e&fromstate=<%=fromstate%>" >Edit a link</a><br>
                        <!--<a href="editor.jsp?user=<%=user%>&state=3m&fromstate=<%=fromstate%>" >Move a link</a><br>-->
                        <a href="editor.jsp?user=<%=user%>&state=3d&fromstate=<%=fromstate%>" >Delete a link</a><br>
                        <!--Move a group<br>-->
                        <a href="editor.jsp?user=<%=user%>&state=3r&fromstate=<%=fromstate%>" >Rename a category</a>
                        <br>
                        <br>
                        <a href="editor.jsp?user=<%=user%>&state=3o&fromstate=<%=fromstate%>" >More options</a>
                        <br><br><br>
                        <a href="user/<%=user%>">Exit</a>
                    </div>
                    <%
		}
		...
		//state 3o, display 'more options', non-basic options
                    ...
                    %>
                    <div style="text-align: center;">
                        <a href="editor.jsp?user=<%=user%>&state=3g&fromstate=<%=fromstate%>" >Change search option</a>
                        <br><br><br>
                        <a href="editor.jsp?user=<%=user%>&state=3p&fromstate=<%=fromstate%>" >Change password</a>
                        <br><br>
                        <a href="editor.jsp?user=<%=user%>&state=3dj&fromstate=<%=fromstate%>" >Delete my account</a>
                        <br><br><br>
                        <a href="editor.jsp?user=<%=user%>&state=2&fromstate=0">Back to main options</a>
                        <br><br>
                        <a href="user/<%=user%>">Exit</a>
                    </div>
                    <%
		...
		//state 3a, display add a link dialog and form
                    ...
                    <p style="text-align: center; padding: 30px 7px;">
                        <a href="editor.jsp?user=<%=user%>&state=2&fromstate=0">Cancel</a>
                    </p>
                    <%
		}
		...
		//state 3a_2, they are adding link, display choices of rows (subcategories) in this category
                    ...
                    if (catradio.equals("null")){
                        ...
                        <p style="text-align: center;">
                            <a href="editor.jsp?user=<%=user%>&state=2&fromstate=3a&linknamenew=<%=URLEncoder.encode(linknamenew, "UTF-8")%>&linkurlnew=<%=URLEncoder.encode(linkurlnew, "UTF-8")%>&catradio=<%=URLEncoder.encode(catradio, "UTF-8")%>&catnew=<%=URLEncoder.encode(catnew, "UTF-8")%>&rowradio=1">add this link to the new category you specified</a>
                        </p>
                        <%
                    }
                    else {
                        ...
                        if (catradio.equals("")){
                            ...
                            if (!rslinksinnocat.next()){ //unique case, give the user an option
                                ...
                                <p style="text-align: center;">
                                    <a href="editor.jsp?user=<%=user%>&state=2&fromstate=3a&linknamenew=<%=URLEncoder.encode(linknamenew, "UTF-8")%>&linkurlnew=<%=URLEncoder.encode(linkurlnew, "UTF-8")%>&catradio=<%=URLEncoder.encode(catradio, "UTF-8")%>&catnew=<%=URLEncoder.encode(catnew, "UTF-8")%>&rowradio=1">add this link</a>
                                </p>
                                <%
                            }
                            else {
                        ...
                        if (!catradio.equals("") || noCatLinks){
                            ...
                            while (rsfirstlinks.next()){
                                %>
                                <p style="text-align: center;">
                                    <a href="editor.jsp?user=<%=user%>&state=2&fromstate=3a_2&linknamenew=<%=URLEncoder.encode(linknamenew, "UTF-8")%>&linkurlnew=<%=URLEncoder.encode(linkurlnew, "UTF-8")%>&catradio=<%=URLEncoder.encode(catradio, "UTF-8")%>&catnew=<%=URLEncoder.encode(catnew, "UTF-8")%>&rowradio=<%=rowradio%>&catrank=<%=rsfirstlinks.getString("cat_rank")%>&subcatrank=<%=rsfirstlinks.getString("sub_cat_rank")%>">
		...
		//state 3g, display search options
			...
			<center><INPUT TYPE=hidden NAME=user value="<%=user%>" >
			<INPUT TYPE=hidden NAME=state value="2" >
			<INPUT TYPE=hidden NAME=fromstate value="<%=fromstate%>" >
			<INPUT TYPE=hidden NAME=searchradio value="<%=searchradio%>" >
		...
		//state 3dj, display confirmation to delete jumpstation
			...
			<p><a href="editor.jsp?user=<%=user%>&state=2&fromstate=<%=fromstate%>" >Yes</a>
			<br><br><br>
